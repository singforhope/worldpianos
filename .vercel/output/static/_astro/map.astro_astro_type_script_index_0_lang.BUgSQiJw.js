import{m as d}from"./mapbox-gl.DMBwQGmc.js";d.accessToken="pk.eyJ1Ijoic2ZoYWRtaW4iLCJhIjoiY2t6bWZnY2VhNWY0djJwdHZhZnpvY3prbSJ9.5vyd64pGtGwl9YfMNFH9eQ";const y=document.createElement("div");y.innerHTML=`
    ...
    `;const v=document.getElementById("map"),s=JSON.parse(v?.dataset.pianos||"[]");console.log("Loaded piano data:",s);console.log("Number of pianos:",s.length);console.log("Piano categories:",s.map(o=>({name:o.name,category:o.category})));const g=localStorage.getItem("userCoordinates");let m=[0,0],h=1.5;if(g)try{const{lat:o,lng:i}=JSON.parse(g);m=[i,o],h=12}catch(o){console.error("Error parsing saved coordinates:",o)}const L=new URLSearchParams(window.location.search),u=L.get("pianoId");let l=null;u&&(l=s.find(o=>o.id===u),l&&(m=l.coordinates,h=15));console.log("Initializing map with center:",m,"zoom:",h);const n=new d.Map({container:"map",style:"mapbox://styles/mapbox/navigation-day-v1",center:m,zoom:h,minZoom:1.5,maxZoom:22});n.addControl(new d.NavigationControl,"top-right");n.on("load",()=>{console.log("Map loaded, adding markers..."),console.log("Pianos data:",s),s.forEach(e=>{console.log("Adding marker for piano:",e.name,"at coordinates:",e.coordinates,"category:",e.category,"type:",e.type);const t=new d.Popup({offset:25}).setHTML(`
                <div class="p-2">
                    <h3 class="font-bold text-lg">${e.name}</h3>
                    <p class="text-sm text-base-content/70">${e.location}</p>
                    <p class="text-sm mt-2">${e.description}</p>
                    <div class="mt-2 flex gap-2">
                        <span class="badge badge-sm">${e.type}</span>
                        <span class="badge badge-sm badge-success">${e.condition}</span>
                        <span class="badge badge-sm badge-info">${e.category}</span>
                    </div>
                    <div class="mt-2 text-xs text-base-content/70">
                        <p>City: ${e.city}</p>
                        <p>Country: ${e.country}</p>
                    </div>
                    <a href="/pianos/${e.id}" target="_blank" class="btn btn-primary btn-sm mt-2 w-full">
                        View Details
                    </a>
                </div>
            `),a=e.category==="airport"?y.cloneNode(!0):w.cloneNode(!0);console.log("Created marker element for:",e.name,"category:",e.category,"element:",a);const c=new d.Marker({element:a,anchor:"center",scale:.5}).setLngLat(e.coordinates).setPopup(t).addTo(n);l&&e.id===l.id&&c.togglePopup(),console.log("Marker added successfully for:",e.name)});function o(e,t,a,c){const p=(a-e)*Math.PI/180,C=(c-t)*Math.PI/180,f=Math.sin(p/2)*Math.sin(p/2)+Math.cos(e*Math.PI/180)*Math.cos(a*Math.PI/180)*Math.sin(C/2)*Math.sin(C/2);return 6371*(2*Math.atan2(Math.sqrt(f),Math.sqrt(1-f)))}function i(e,t){const c=s.map(r=>({...r,distance:o(e,t,r.coordinates[1],r.coordinates[0])})).sort((r,p)=>r.distance-p.distance).slice(0,3);n.getSource("nearest-pianos")&&(n.removeLayer("nearest-pianos"),n.removeSource("nearest-pianos")),n.addSource("nearest-pianos",{type:"geojson",data:{type:"FeatureCollection",features:c.map(r=>({type:"Feature",properties:{},geometry:{type:"LineString",coordinates:[[t,e],r.coordinates]}}))}}),n.addLayer({id:"nearest-pianos",type:"line",source:"nearest-pianos",layout:{"line-join":"round","line-cap":"round"},paint:{"line-color":"#800000","line-width":2,"line-dasharray":[2,2],"line-opacity":.6}})}if(g)try{const{lat:e,lng:t}=JSON.parse(g);i(e,t)}catch(e){console.error("Error parsing saved coordinates:",e)}window.addEventListener("storage",e=>{if(e.key==="userCoordinates"&&e.newValue)try{const{lat:t,lng:a}=JSON.parse(e.newValue);i(t,a)}catch(t){console.error("Error parsing new coordinates:",t)}}),window.addEventListener("locationChanged",e=>{const{lat:t,lng:a}=e.detail;n.flyTo({center:[a,t],zoom:12,duration:2e3,essential:!0}),i(t,a)})});n.on("error",o=>{console.error("Map error:",o)});
