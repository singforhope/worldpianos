-- Migration to update the RLS policy for event_reports table
-- This makes the policy more permissive for testing purposes

-- First, drop the existing policy if it exists
DROP POLICY IF EXISTS "Authenticated users can create event reports" ON event_reports;

-- Create a new, more permissive policy
CREATE POLICY "Anyone can create event reports"
ON event_reports 
FOR INSERT 
...
COMMENT ON POLICY "Anyone can create event reports" ON event_reports
